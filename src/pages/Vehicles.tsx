...
import { Search, MapPin, Lock, Unlock, Battery, X, History, Zap, Filter, Crosshair, Navigation } from 'lucide-react';
...
export default function Vehicles() {
  ...
  const locationVehicle = showLocation ? vehicles.find(v => v.id === showLocation) : null;
                ...
                {filteredVehicles.map((v) => (
                  <tr
                    ...
                    className={clsx(
                      ...
                      selectedVehicleId === v.id && 'bg-blue-50'
                    )}
                  >
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); setShowLocation(v.id); }}
        ...
        <div className="w-96 bg-white rounded-xl shadow-sm border border-slate-100 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setShowLocation(selectedVehicle.id)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors font-medium text-sm"
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-[520px] max-w-[90vw] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-800">{locationVehicle.code} 位置信息</h3>
                  <p className="text-xs text-slate-500">实时定位</p>
                </div>
              </div>
              <button
            ...
            <div className="px-6 py-4">
              <div className="relative w-full h-64 bg-gradient-to-br from-blue-50 via-cyan-50 to-emerald-50 rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <div className="relative">
                    <div className="w-12 h-12 bg-red-500 rounded-full flex items-center justify-center shadow-lg shadow-red-500/40 animate-pulse">
                      <MapPin className="w-6 h-6 text-white" />
                    </div>
                    <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-red-500 rotate-45 -z-10" />
                  </div>
                  <div className="mt-3 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-sm border border-slate-200">
                    <p className="text-xs font-semibold text-slate-700">{locationVehicle.code}</p>
                  </div>
                </div>
                <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2 py-1 bg-white/80 backdrop-blur-sm rounded-md shadow-sm border border-slate-200">
                  <Crosshair className="w-3.5 h-3.5 text-blue-500" />
                  <span className="text-xs font-medium text-slate-600">实时定位</span>
                </div>
                <div className="absolute bottom-3 right-3 flex items-center gap-1 px-2 py-1 bg-white/80 backdrop-blur-sm rounded-md shadow-sm border border-slate-200">
                  <Navigation className="w-3.5 h-3.5 text-emerald-500" />
                  <span className="text-xs text-slate-600">{locationVehicle.lat.toFixed(4)}°N, {locationVehicle.lng.toFixed(4)}°E</span>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 pb-4 space-y-2">
              <div className="flex items-center justify-between py-2.5 px-4 bg-slate-50 rounded-lg">
                <span className="text-sm text-slate-500">纬度 (Lat)</span>
                <span className="text-sm font-semibold text-slate-800 font-mono">{locationVehicle.lat.toFixed(6)}</span>
              </div>
              <div className="flex items-center justify-between py-2.5 px-4 bg-slate-50 rounded-lg">
                <span className="text-sm text-slate-500">经度 (Lng)</span>
                <span className="text-sm font-semibold text-slate-800 font-mono">{locationVehicle.lng.toFixed(6)}</span>
              </div>
              <div className="flex items-center justify-between py-2.5 px-4 bg-slate-50 rounded-lg">
                <span className="text-sm text-slate-500">所属站点</span>
                <span className="text-sm font-semibold text-slate-800">{getStationName(locationVehicle.stationId)}</span>
              </div>
              <div className="flex items-center justify-between py-2.5 px-4 bg-slate-50 rounded-lg">
                <span className="text-sm text-slate-500">车辆状态</span>
                <StatusBadge status={locationVehicle.status} />
              </div>
            </div>
            ...
            <div className="px-6 pb-6">
              <button
                onClick={() => setShowLocation(null)}
                className="w-full py-2.5 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors font-medium text-sm"
              >
                关闭
              </button>
            </div>
